...
import { Left, Right, isLeft } from "./types";
...
export function extractBlock(userscript) {
    const lineComments = (extractComments(userscript)
        .filter(c => c.type === "LineComment")
        .map(c => c.raw));
    if (lineComments.length === 0) {
        return Left("NO_LINE_COMMENTS" /* NO_LINE_COMMENTS */);
    }
    else {
        ...
        if (startTagIndex < 0) {
            return Left("NO_START_TAG" /* NO_START_TAG */);
        }
        else if (endTagIndex < 0) {
            return Left("NO_END_TAG" /* NO_END_TAG */);
        }
        else if (endTagIndex < startTagIndex) {
            return Left("END_TAG_BEFORE_START_TAG" /* END_TAG_BEFORE_START_TAG */);
        }
        else {
...
export function parseLine(line) {
    const match = line.match(new RegExp(r `^\s*` + TAG_PREFIX + r `([a-z\-]+)` + r `(?:\s*|\s+(\S.*))$`, "i"));
    if (match === null) {
        return Left(line);
    }
    else {
        const key = match[1];
        const value = match[2];
        return Right({ key, value: value ? value.trimEnd() : true });
    }
}
