...
export const DEFAULT_WARNINGS = [
    entries => (!entries.some(entry => entry.key === KEY.match || entry.key === KEY.include)
        ? [{
                summary: Msg.noMatchOrIncludeSummary,
                description: Msg.noMatchOrIncludeDescription,
            }]
        : []),
    entries => (entries.some(entry => entry.key === KEY.include && !isIncludePattern_regex(entry.value))
        ? [{
